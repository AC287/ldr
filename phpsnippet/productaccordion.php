              ...
              if(!empty($s3_category[0]->s3)) {
                ...
                foreach($s3_category as $each_s3) {
                  ...
                  if(!empty($s4_category[0]->s4)) {
                    echo "<div class='custaccordion s3-$s3class'><img class='chev' src='http://files.coda.com.s3.amazonaws.com/imgv2/icons/chev-right.png'>";
                    echo "<a class='custaccordion-s3a' href='".home_url()."/products/categories/?m0=".urlencode($each_mc->m0)."&s1=".urlencode($each_s1->s1)."&s2=".urlencode($each_s2->s2)."&s3=".urlencode($each_s3->s3)."'>".$each_s3->s3."</a></div>";
